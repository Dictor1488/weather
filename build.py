...
class ElapsedFormatter(logging.Formatter):
    """A logging formatter that includes elapsed time since initialization."""
...
def copytree(source, destination, ignore=None):
    ...
    ignored_names = ignore(str(source_path), names) if ignore else set()
...
def build_flash(config, args):
    ...
    for file_path in files_to_process:
        ...
        jsfl_file = pathlib.Path('build-%s.jsfl' % rand_str(5))
        document_uri = file_path.resolve().as_uri()
        log_file_uri = log_path.resolve().as_uri()

        jsfl_content = 'fl.publishDocument("%s", "Default");\n' % document_uri
        jsfl_content += 'fl.compilerErrors.save("%s", false, true);\n' % log_file_uri
        ...
        if log_data:
            raise RuntimeError('Failed flash publish %s\n%s' % (file_path, log_data))
        else:
...
def build_python(config):
    ...
    if not py2_path.is_file():
        raise FileNotFoundError('Configured Python 2 executable not found: %s' % config.software.python)
    ...
    for file_path in python_source_dir.rglob('*.py'):
        try:
            ...
            if not pyc_path.is_file():
                failed_files.append('%s (py_compile returned success, but .pyc not found)' % file_path)
    ...
    if failed_files:
        raise RuntimeError(
            'Python compilation failed for %d file(s):\n%s' % (
                len(failed_files),
                '\n'.join(failed_files)
            )
        )
...
def main():
    ...
    package_name = '%s_%s.wotmod' % (config.info.id, config.info.version)
    ...
    if args.ingame:
        ...
        if not wot_packages_dir.is_dir():
            raise FileNotFoundError('WoT mods folder not found: %s' % wot_packages_dir)
    ...
    if args.distribute:
        ...
        zip_name = '%s_%s.zip' % (config.info.id, config.info.version)
