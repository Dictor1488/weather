...
try:
    from weather.controller import g_controller
except Exception:
    g_controller = None
...
def _register_weather_view():
    ...
    if _VIEW_REGISTERED:
        return
    try:
        from weather.window import WeatherWindowMeta
...
def open_weather_window(*args, **kwargs):
    ...
    def _load():
        try:
            try:
                from weather.window import show_existing_window
...
def _register_mods_list_entry_now():
    ...
    def _try_register(method_name):
        ...
        attempts = (
            lambda: method(**entry),
            lambda: method(entry['id'], entry['name'], entry['description'], entry['icon'],
                           entry['enabled'], entry['login'], entry['lobby'], entry['callback']),
...
_orig_handleKeyEvent = None


def _install_key_hook():
    global _KEY_HOOK_INSTALLED, _orig_handleKeyEvent
    if not IN_GAME or _KEY_HOOK_INSTALLED:
        return
    try:
        import AvatarInputHandler as _AIH
        cls = _AIH.AvatarInputHandler
        if not hasattr(cls, 'handleKeyEvent'):
            return
        orig = cls.handleKeyEvent
        if getattr(orig, '_weather_patched', False):
            # already patched by us in a previous init (e.g. reload without restart)
            _KEY_HOOK_INSTALLED = True
            return
        _orig_handleKeyEvent = orig  # save for clean restore in fini()
        ...
        _KEY_HOOK_INSTALLED = True
        _log().info('Key hook: AvatarInputHandler.handleKeyEvent installed OK')
    except Exception:
        _log().exception('Failed to install AvatarInputHandler key hook')


def _uninstall_key_hook():
    global _KEY_HOOK_INSTALLED, _orig_handleKeyEvent
    if not _KEY_HOOK_INSTALLED or _orig_handleKeyEvent is None:
        return
    try:
        import AvatarInputHandler as _AIH
        current = getattr(_AIH.AvatarInputHandler, 'handleKeyEvent', None)
        # Only restore if our wrapper is still the active one (no one else re-patched after us)
        if current is not None and getattr(current, '_weather_patched', False):
            _AIH.AvatarInputHandler.handleKeyEvent = _orig_handleKeyEvent
            _log().info('Key hook: AvatarInputHandler.handleKeyEvent restored OK')
        else:
            _log().warning('Key hook: skipped restore — method was re-patched by another mod')
    except Exception:
        _log().exception('Failed to uninstall AvatarInputHandler key hook')
    finally:
        _orig_handleKeyEvent = None
        _KEY_HOOK_INSTALLED = False
